...
template <typename T>
const std::optional<T> peek(std::unique_ptr<Vrtl_lod4_dut> &dut,
                            const int which) {
...
template <typename T>
void poke(std::unique_ptr<Vrtl_lod4_dut> &dut, const int which, const T val) {
  switch (which) {
  case 0:
    dut->rx_enable = val;
    return;
