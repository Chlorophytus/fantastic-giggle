...
namespace rtl_shift2 {
const char *const *get_peekables();
const char *const *get_pokeables();
template <typename T>
const std::optional<T> peek(std::unique_ptr<Vrtl_shift2_dut> &dut,
                            const int which) {
...
template <typename T>
void poke(std::unique_ptr<Vrtl_shift2_dut> &dut, const int which, const T val) {
  switch (which) {
  case 0:
    dut->rx_enable = val;
    return;
