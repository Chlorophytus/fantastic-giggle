#pragma once
#include "Vrtl_carry4_alu2_dut.h"
#include "config.hpp"
#include <cstdint>
#include <filesystem>
#include <lua.hpp>
...
namespace rtl_carry4_alu2 {
const char *const *get_peekables();
const char *const *get_pokeables();
template <typename T>
const std::optional<T> peek(std::unique_ptr<Vrtl_carry4_alu2_dut> &dut,
                            const int which) {
...
template <typename T>
void poke(std::unique_ptr<Vrtl_carry4_alu2_dut> &dut, const int which, const T val) {
  switch (which) {
  case 0:
    dut->rx_enable = val;
    return;
